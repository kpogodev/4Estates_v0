  ...
  return (
    <form className={`${className} relative`} onSubmit={handleSubmit}>
      <label className=' relative mt-1 h-full flex justify-center items-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-slate-100 transition-colors '>
        ...
        <input type='file' onChange={handleChange} className='block absolute top-0 left-0 w-full h-full opacity-0 cursor-pointer' multiple />
      ...
      {files.length > 0 && (
        <button type='submit' className='btn btn-sm btn-primary absolute !left-1/2 !bottom-2 !-translate-x-1/2 text-xl rounded-full'>
          <MdUpload /> Upload
        </button>
      )}
    </form>
