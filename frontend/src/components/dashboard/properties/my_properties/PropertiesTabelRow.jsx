...
function PropertiesTabelRow({ property }) {
  return (
    <motion.div variants={forViewSwitcherItems} className='w-full grid grid-cols-12 place-items-center'>
      <Link to={`/user/manage-property/${property._id}`} className='col-span-6 justify-self-start'>
      ...
      <div className='col-span-2'>
        <Link to={`/user/manage-property/${property._id}`} className='btn btn-primary btn-xs'>
          details
        </Link>
      </div>
    </motion.div>
