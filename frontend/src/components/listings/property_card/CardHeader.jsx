...
const CardHeader = ({ type, price, is_premium }) => {
  ...
  return (
    <div
      className={`w-full p-3 flex flex-col items-start md:absolute md:left-0 md:bottom-0 md:border-r md:max-w-[270px] lg:max-w-[530px] ${variableStyleBackground}`}
    >
      {type === 'rent' ? (
        <>
      ...
      {is_premium && <span className='absolute right-0 top-0 w-20 h-full p-2 flex items-center font-semibold text-white uppercase text-center text-sm leading-tight bg-[rgba(0,0,0,0.1)]'>Premium Listing</span>}
    </div>
  )
}
